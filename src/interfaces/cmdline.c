...
GtkWidget *label;
GtkWidget *netListlabel;

void appendNetList(char data[]){
    if(check++==0){
        netlist=malloc(sizeof(char)*100);
        strcpy(netlist,"NET-LIST: ");
        strcat(netlist,data);
        strcat(netlist,",  ");
    }else if((check++)%8==0){
        netlist=realloc(netlist,strlen(netlist)+10);
        strcat(netlist,data);
        strcat(netlist,"\n");
    }else{
        check++;
    ...
    gtk_label_set_text(netListlabel,netlist);
}
void setLabel(GtkWidget *labeltemp){
    label=labeltemp;
}
void setNetListLabel(GtkWidget *labeltemp){
    netListlabel=labeltemp;
}
